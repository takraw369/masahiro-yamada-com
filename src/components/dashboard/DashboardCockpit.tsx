...
type CockpitData = {
  ...
  project: {
    available: boolean;
    primaryFocus: ProjectItem | null;
    focus: ProjectItem[];
    bottleneck: ProjectItem | null;
    blocked: ProjectItem[];
    signals: ProjectSignals | null;
  };
  evidence: {
    available: boolean;
    items: Array<{ id:string; title:string; type:string; quality:string; status:string; tags:string[]; occurredAt:string|null; updatedAt:string|null }>;
  };
};
...
const shortDate = (value: string | null | undefined) => {
  ...
  return Number.isFinite(date.getTime()) ? new Intl.DateTimeFormat('ja-JP', { month:'numeric', day:'numeric' }).format(date) : value;
};
...
export default function DashboardCockpit() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/dashboard/cockpit', { cache:'no-store' });
  ...
  const submitInput = async () => {
    ...
    try {
      if (inputMode === 'evidence') {
        ...
        setInputStatus(inputMode === 'comment' ? 'コメントを残した' : inputMode === 'create' ? '制作依頼をInboxへ置いた' : '指示をInboxへ置いた');
      }
      setInput('');
    } catch {
  ...
  const renderWidget = (id: WidgetId) => {
    if (id === 'project') return <>
      <div className="widget-head"><div><span>PROJECT</span><h2>今、流れを動かすもの</h2></div><a href="/dashboard/os">詳細 →</a></div>
      {loading ? <p className="loading">Project OSを読んでる…</p> : project?.primaryFocus ? <>
        <article className="primary-focus">
          <div className="project-meta"><b>{project.primaryFocus.priority || '—'}</b><span>{project.primaryFocus.status || 'NOW'}</span>{project.primaryFocus.dueDate && <em>期限 {shortDate(project.primaryFocus.dueDate)}</em>}</div>
          <h3>{project.primaryFocus.title}</h3>
          <p>{project.primaryFocus.nextAction || 'Next Action未設定'}</p>
        </article>
        <div className="focus-list">
          {(project.focus || []).slice(1, 4).map(item => <article key={item.id || item.title}>
            <div><b>{item.priority || '—'}</b><strong>{item.title}</strong></div>
            <span>{item.status || item.type}</span>
          </article>)}
        </div>
      </> : <div className="empty"><strong>Project OS unavailable</strong><span>正本は変更せず、詳細ページから確認。</span></div>}
    </>;

    if (id === 'wait') return <>
      <div className="widget-head"><div><span>WAIT / BOTTLENECK</span><h2>詰まりだけ見る</h2></div><a href="/dashboard/os">開く →</a></div>
      <div className="metric-grid">
        <div><b>{num(signals?.blocked_high_priority)}</b><span>P0/P1 WAIT</span></div>
        <div><b>{num(signals?.overdue_actionable)}</b><span>期限超過</span></div>
        <div><b>{num(signals?.stale_actionable)}</b><span>STALE</span></div>
      </div>
      {project?.bottleneck && <article className="bottleneck"><span>BOTTLENECK</span><strong>{project.bottleneck.title}</strong><p>{project.bottleneck.nextAction || 'Next Action未設定'}</p></article>}
    </>;
    ...
    if (id === 'command') return <>
      ...
      <div className="input-foot"><span>{inputStatus || '正式変更・公開はここでは自動実行しない。'}</span><button type="button" onClick={submitInput} disabled={!input.trim() || sending}>{sending ? '保存中…' : '置く →'}</button></div>
    </>;
    ...
    if (id === 'evidence') return <>
      ...
      <div className="evidence-list">
        {(data?.evidence.items || []).slice(0, 4).map(item => <article key={item.id || item.title}><div><strong>{item.title}</strong><small>{item.type || item.status || 'evidence'} · {shortDate(item.updatedAt || item.occurredAt)}</small></div><span>{item.quality || 'raw'}</span></article>)}
        {!loading && !data?.evidence.items?.length && <div className="empty"><strong>まだ表示するEvidenceなし</strong><span>QUICK INPUTから追加できる。</span></div>}
      </div>
    </>;

    if (id === 'calendar') return <>
      <div className="widget-head"><div><span>CALENDAR</span><h2>時間へ落とす</h2></div><a href="/dashboard/schedule">開く →</a></div>
      <p className="widget-copy">固定予定・締切・家族時間はCalendar側を正本にして、ここでは「今日触る入口」だけ置く。</p>
    ...
    return <>
      ...
      <p className="widget-copy">Projectを見れば済むことは質問にしない。複数の場所を跨ぐ未決定だけChoice Labへ。</p>
