...
export const GET = async ({ locals }: APIContext) => {
  ...
  try {
    ...
    const state = await supabaseRpc<Record<string, unknown>>(env, 'masa_choice_state_get_v1', {
      p_owner_key: ownerKey,
    });
    return json({ ok: true, state: state || {}, storage: 'supabase' });
  } catch {
    return json({ ok: false, state: {}, storage: 'unavailable', error: 'primary_storage_unavailable' }, 503);
  }
};
...
export const POST = async ({ request, locals }: APIContext) => {
  ...
  if (JSON.stringify(body.state).length > 180000) {
    return json({ ok: false, error: 'state_too_large' }, 413);
  }

  try {
    const ownerKey = await getDashboardOwnerKey(env);
    await supabaseRpc<boolean>(env, 'masa_choice_state_set_v1', {
      p_owner_key: ownerKey,
      p_state: body.state,
    });
    return json({ ok: true, storage: 'supabase', savedAt: new Date().toISOString() });
  } catch {
    return json({ ok: false, storage: 'unavailable', error: 'primary_storage_unavailable' }, 503);
  }
};
