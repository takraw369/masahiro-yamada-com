...
export type SourceType =
  | Extract<KnowledgeSourceType, "web" | "youtube" | "research" | "pdf">
  | "x";
...
export const isInbox = (item: KnowledgeItem) =>
  item.status === "inbox" || item.status === "review";
export function normalizeUrl(value: string): string {
  ...
  if (
    ...
    throw new Error("HTTP / HTTPS のURLを入力してください。");
  url.hash = "";
  return url.href;
}
...
export function selectItems(
  ...
  return snapshot.items
    .filter((item) => {
      ...
      if (query.project_id && !item.project_ids.includes(query.project_id))
        return false;
      if (query.status && item.status !== query.status) return false;
      if (query.attention === "review" && item.status !== "review")
        return false;
      if (query.attention === "high" && (item.asset_score ?? -1) < 80)
        return false;
      if (query.attention === "ready" && item.status !== "ready") return false;
      if (query.attention === "unconnected" && item.project_ids.length)
        return false;
      if (
        query.attention === "dormant" &&
        ((item.asset_score ?? -1) < 80 ||
          Date.parse(item.updated_at) > dormantBefore.getTime())
      )
        return false;
      const haystack = [
        item.title,
        ...
        ...snapshot.projects
          .filter((p) => item.project_ids.includes(p.id))
          .map((p) => p.name),
        ...snapshot.tags
