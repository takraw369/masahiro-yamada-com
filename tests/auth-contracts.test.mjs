...
test('middleware exempts only exact auth bootstrap paths and still requires same-origin mutations', async () => {
  for (const path of ['/api/dashboard/google-login', '/api/dashboard/reset-password', '/api/dashboard/password-login']) {
...
const { POST: passwordLogin } = await import('../src/pages/api/dashboard/password-login.ts');
test('JSON password login rejects malformed input and non-boolean verification before issuing a session', async (t) => {
  for (const body of ['null', '[]', '{', '{"password":123456789012}', '{}']) {
    const ctx = context('/api/dashboard/password-login', { body });
    assert.equal((await passwordLogin(ctx)).status, 400);
    assert.equal(ctx.writes.length, 0);
  }
  for (const valid of [false, null, 'false', {}, [true], true]) {
    await t.test(JSON.stringify(valid), async (t) => {
      t.mock.method(globalThis, 'fetch', async (url, options) => {
        assert.ok(url.endsWith('/rpc/verify_dashboard_login_password'));
        assert.deepEqual(JSON.parse(options.body), { p_password: password });
        return Response.json(valid);
      });
      const ctx = context('/api/dashboard/password-login', { body: JSON.stringify({ password }) });
      const response = await passwordLogin(ctx);
      assert.equal(response.status, valid === true ? 200 : 401);
      if (valid === true) await assertSessionCookie(ctx);
      else assert.equal(ctx.writes.length, 0);
    });
  }
});
